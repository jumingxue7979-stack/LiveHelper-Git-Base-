...
이 문서는 Windows 쪽에서 먼저 남긴 2026-05-17 중간 작업일지다. 이후 Mac에서 비교 리포트 하단 `참고사항` 잘림을 `2f12f7d`로 수정했고, 오늘 최종 기준은 아래 문서를 우선한다.
...
따라서 이 문서 안의 `현재 최신 커밋 f69ef08`이나 `비교 리포트 하단 잘림은 맥에서 수정 필요`라는 표현은 중간 시점 기록이다. 최종 상태에서는 비교 리포트 하단 잘림 수정까지 완료되어 GitHub에 올라갔다.
